...
const C = {
  ui: {
    ...
    verifier: {
      ...
      gameHash: 'Game hash — round result hash',
      gameHashPh: 'the round’s hash, e.g. 0a1b2c… (hex)',
      salt: 'Salt (optional)',
      saltPh: 'public salt, if your casino HMACs the hash',
  ...
  games: {
    ...
    crash: {
      name: 'Crash', resultLabel: 'Multiplier', tagline: 'The round’s crash multiplier, from the game hash.',
      intro: 'Crash is multiplayer: one shared multiplier per round, derived from that round’s hash — not from your personal seed and nonce. Faircheck recomputes the exact crash point from the hash the casino published, with the house edge exposed.',
      howItWorks: [
        'Before the round, the casino commits to a hash; when the round ends, that hash (its place in a public hash chain) is revealed.',
        'A 13-hex-digit slice of the hash becomes an integer h, and the crash point is floor((100·2^52 − h) / (2^52 − h)) / 100.',
        'A small fraction of rounds — about the house edge, e.g. 1 in 101 at 1% — instantly busts at 1.00×; that is where the house edge lives.',
      ],
      faq: [
        ['Why a game hash, not a server seed and nonce?', 'Crash is one shared game for everyone in the round, so it cannot depend on any single player’s client seed. The whole round is fixed by one hash the casino commits to in advance.'],
        ['My casino uses a salt — what is it?', 'Some operators HMAC the round hash with a public salt (often a future Bitcoin block hash) before computing the multiplier. Paste it in the optional Salt field; leave it empty otherwise.'],
      ],
      note: 'Heads up: Crash bakes {casino}’s house edge into the formula via an instant-bust chance. If your result is off, set the house edge to your casino’s value (1% is standard).',
    },
  },
