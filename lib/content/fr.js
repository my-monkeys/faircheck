...
const C = {
  ui: {
    ...
    verifier: {
      ...
      gameHash: 'Hash du jeu — hash du résultat de la manche',
      gameHashPh: 'le hash de la manche, ex. 0a1b2c… (hex)',
      salt: 'Salt (optionnel)',
      saltPh: 'salt public, si votre casino applique un HMAC au hash',
  ...
  games: {
    ...
    crash: {
      name: 'Crash', resultLabel: 'Multiplicateur', tagline: 'Le multiplicateur de crash de la manche, à partir du hash du jeu.',
      intro: 'Crash est multijoueur : un seul multiplicateur partagé par manche, dérivé du hash de cette manche — pas de votre server seed et de votre nonce personnels. Faircheck recalcule le point de crash exact à partir du hash que le casino a publié, avec l’avantage de la maison exposé.',
      howItWorks: [
        'Avant la manche, le casino s’engage sur un hash ; quand la manche se termine, ce hash (sa place dans une chaîne de hashs publique) est révélé.',
        'Une tranche de 13 chiffres hexadécimaux du hash devient un entier h, et le point de crash est floor((100·2^52 − h) / (2^52 − h)) / 100.',
        'Une petite fraction des manches — environ l’avantage de la maison, ex. 1 sur 101 à 1 % — explose instantanément à 1.00× ; c’est là que vit l’avantage de la maison.',
      ],
      faq: [
        ['Pourquoi un hash du jeu, et non une server seed et un nonce ?', 'Crash est un seul jeu partagé par tous les joueurs de la manche, il ne peut donc pas dépendre de la client seed d’un joueur en particulier. Toute la manche est fixée par un seul hash sur lequel le casino s’engage à l’avance.'],
        ['Mon casino utilise un salt — qu’est-ce que c’est ?', 'Certains opérateurs appliquent un HMAC au hash de la manche avec un salt public (souvent un futur hash de bloc Bitcoin) avant de calculer le multiplicateur. Collez-le dans le champ Salt optionnel ; sinon, laissez-le vide.'],
      ],
      note: 'À noter : Crash intègre l’avantage de la maison de {casino} dans la formule via une probabilité d’explosion instantanée. Si votre résultat est faux, réglez l’avantage de la maison sur la valeur de votre casino (1 % est la norme).',
    },
  },
